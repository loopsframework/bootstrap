...
if [ ! $DISABLE_HTTP ]; then
    if [ ! -e "/etc/nginx/sites-available/$HOSTNAME" ]; then
        cp /nginx.template "/etc/nginx/sites-available/$HOSTNAME"
...
if [ "$ENABLE_HTTPS" ]; then
    if [ ! -e "/etc/nginx/sites-available/$HOSTNAME-ssl" ]; then
        cp /nginx.template-ssl "/etc/nginx/sites-available/$HOSTNAME-ssl"
...
if [ $ENABLE_JOBS ]; then
    ...
    sed -i -- "s/autostart=false/autostart=true/g" "/etc/supervisor/conf.d/redis.conf"
fi
